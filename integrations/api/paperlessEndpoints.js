    ...
    static registerRoutes(app, getSettings) {
                ...
                if (apiToken === TOKENMASK) {
                    try {
                        ...
                        if (paperlessConfig?.apiToken && paperlessConfig.apiToken !== TOKENMASK) {
                            ...
                            return res.status(400).json({ 
                                success: false, 
                                error: 'No stored API token found. Please enter a new token.' 
                            });
                        }
                    } catch (error) {
                        return res.status(400).json({ 
                            success: false, 
                            error: 'Failed to retrieve stored API token' 
                        });
                    }
                }

                const result = await PaperlessEndpoints.testConnection({ hostUrl, apiToken });
        ...
        app.get(BASE_PATH + '/api/paperless/search', async (req, res) => {
            try {
                ...
                const results = await PaperlessEndpoints.searchDocuments(
                    config, 
                    query, 
                    parseInt(page), 
                    parseInt(pageSize)
        ...
        app.get(BASE_PATH + '/api/paperless/document/:id/info', async (req, res) => {
            try {
                ...
                const info = await PaperlessEndpoints.getDocumentInfo(config, documentId);
                res.json(info);
            } catch (error) {
                console.error('Failed to get document info:', error);
                res.status(500).json({ error: error.message });
        ...
        app.get(BASE_PATH + '/api/paperless/document/:id/download', async (req, res) => {
            try {
                ...
                const response = await PaperlessEndpoints.downloadDocument(config, documentId);
...
module.exports = PaperlessEndpoints;
